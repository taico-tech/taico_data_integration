...
def taico_facebook_data_etl_pipeline():
    ...
    with TaskGroup("loading", tooltip="Loading tasks to move data to staging and production") as loading:
        ...
        with TaskGroup("switch_to_production", tooltip="Switch staging data to production") as switch_to_production:
            ...
            backup_production_table = BashOperator(
                task_id='backup_production_table',
                bash_command=(
                    'cd /usr/local/airflow/include/dbt && '
                    'dbt run-operation backup_table --args \'{"project_id": "' + PROJECT_ID + '", "dataset_id": "' + DATASET_ID + '", "source_table": "main_taico_production", "backup_table": "main_taico_production_backup"}\''
                )
            )
            ...
            backup_production_table >> truncate_production_table >> insert_data_to_production
    ...
    with TaskGroup("production_data_validation", tooltip="Validation tasks for production data") as production_data_validation:
        ...
        rollback_production_table = BashOperator(
            task_id='rollback_production_table',
            bash_command=(
                'cd /usr/local/airflow/include/dbt && '
                'dbt run-operation restore_table --args \'{"project_id": "' + PROJECT_ID + '", "dataset_id": "' + DATASET_ID + '", "source_table": "main_taico_production_backup", "target_table": "main_taico_production"}\''
            ),
            trigger_rule=TriggerRule.ONE_FAILED
        )

        # Connect rollback to all validation tasks
        [validate_production_table_schema,
         validate_production_table_row_count,
         validate_production_table_null_values,
         validate_production_table_duplicates] >> rollback_production_table
